...
class SqliteSpotifyClientCredentialsRepository implements
    SpotifyCredentialsRepository
{
    ...
    public function store(SpotifyCredentials $credentials): bool
    {
        ...
        $retVal = $dbh->exec(
            'CREATE TABLE IF NOT EXISTS spotify_client_token
                                (
        ...
        $stmt = $dbh->prepare(
            'REPLACE INTO spotify_client_token (id, token_data, iv)
    ...
    public function get(): ?SpotifyCredentials
    {
        ...
        $selectStmt = $dbh->query(
            'SELECT token_data, iv FROM spotify_client_token;',
            PDO::FETCH_ASSOC
        );
        if (!$selectStmt) {
            return null;
        ...
        $tokenAndTimestampDataJson = json_decode(
            openssl_decrypt(
                $tokenDataEncrypted,
                'aes-256-cbc',
                $password,
