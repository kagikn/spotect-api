...
class TokenFetchingUseCaseTest extends TestCase
{
    ...
    public function testTokenFetchAndCache(SpotifyAuthApi $authApi)
    {
        ...
        $this->assertEquals(
            $firstStoredCredentials->getAccessToken(),
            $firstCredentials->getAccessToken()
        );
        $this->assertEquals(
        ...
        $this->assertEquals(
            $firstStoredCredentials->getAccessToken(),
            $secondCachedCredentials->getAccessToken()
        );
        $this->assertEquals(
            $firstStoredCredentials->getAccessTokenExpirationTimestamp(),
        ...
        $this->assertNotEquals(
            $firstStoredCredentials->getAccessToken(),
            $thirdCredentials->getAccessToken()
        );
    }
     ...
     */
    public function testErrorWhenFailedToFetchAvailableCachedTokenOrNewToken(
        SpotifyAuthApi $authApi
    ) {
        $clientCredentialsRepository = new InMemoryClientCredentialsRepository(
        );
        $logger = new FakeLogger();
